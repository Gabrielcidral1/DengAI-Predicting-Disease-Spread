...
```{r}
pacman:: p_load(readr, h2o, dplyr,ggplot2, caret, imputTS, randomForest, prophet, data.table, lubridate, MMWRweek)
...
# Train a random forest using that mtry
rf_reg<-randomForest(y=sj$total_cases,x=sj[ , head(unique(x$names),6)],importance=T,method="rf", ntree=100, mtry=8)
...
rf_reg<-randomForest(y=iq$total_cases,x=iq[2:23],importance=T,method="rf")
...
# Train a random forest using that mtry
rf_reg<-randomForest(y=iq_tr$total_cases,x=iq_tr[ , head(unique(x$names),6)],importance=T,method="rf", ntree=100, mtry=6)
...
# Train a random forest using that mtry
rf_reg<-randomForest(y=iq$total_cases,x=iq[ , head(unique(x$names),6)],importance=T,method="rf", ntree=100, mtry=8)
...
pred_iq_te <- predict(rf_reg, iq_te)
...
names(submission_format)

submission_format$day <- calculate_start_of_week(submission_format$weekofyear, year = submission_format$year)


forecast$ds <- as.Date(forecast$ds)

left_join(submission_format, y = forecast[ , c("ds","weekofyear","year", "yhat")], by = c("year","weekofyear"))
