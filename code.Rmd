...
rf_reg <- randomForest(y=sj$total_cases,x=sj[,2:14],importance=T,method="rf") # starting in 2 to not use the variable "type"
...
rf_reg<-randomForest(y=sj_tr$total_cases,x=sj_tr[ , head(unique(x$names),6)],importance=T,method="rf", ntree=100, mtry=6)
